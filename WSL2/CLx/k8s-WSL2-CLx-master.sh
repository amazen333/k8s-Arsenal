...
#    Example:
#      sudo ./k8s-WSL2-CLxMaster-bootstrap.sh
#
#  Notes:
#    - Designed for reproducibility and onboarding in cloud or WSL2 environments.
#    - For worker nodes, use the `kubeadm join` command output after init.
#    - Tested on Ubuntu and Debian 12.
#
...
echo "==> Kubernetes master bootstrap (Debian/Ubuntu)"

# --- Verify systemd (required for kubelet/containerd) ---
...
# --- Disable swap (skip fstab edit if absent) ---
sudo swapoff -a || true
if [ -f /etc/fstab ]; then
...
# Ensure kubelet uses systemd cgroups
...
# --- Pre-pull Kubernetes images (aligns pause to 3.9) ---
sudo kubeadm config images pull
sudo ctr images pull registry.k8s.io/pause:3.9 || true
sudo ctr images ls | grep pause || true

# --- Initialize master node ---
# For resilience, recommend running kubeadm inside tmux:
# tmux new -s init 'sudo kubeadm init --pod-network-cidr=10.244.0.0/16'
